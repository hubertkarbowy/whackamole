...
void all_transitions_and_rewards(unsigned short int curr_state,
                                 ...
                                 char* base3_buf,
                                 int num_holes) {
     to_base3_buf(curr_state, base3_buf);
     for (int i=0; i<num_holes; i++) {
         ...
         transitions[i] = base3_to_int(base3_buf); // state after hitting the ith hole
     ...
     to_base3_buf(curr_state, base3_buf); // restore current state in the base3 buffer
     transitions[num_holes] = curr_state; // transition to self after NOT hitting anything...
     rewards[num_holes] = is_terminal_state(base3_buf) == true ? REWARD : IDLE_BUT_WRONG;
}
