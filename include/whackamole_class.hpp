...
class WhacQaMole {
    ...
    char** Q; /*!< a 2D array of state-reward history. Rewards/penalties are within the range of -128 to 127 to conserve space on Arduino */
    float gamma;
    char* temp_base3_buf; /*!< We create these buffers once and for all in the constructor and then reuse them. This is less elegant than creating and destroying them several times, but more efficient. */
    unsigned short int* temp_transitions; 
    ...
    void set_current_state(unsigned short int new_state); // only for simulation and debugging
